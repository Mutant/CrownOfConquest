...
if ($ARGV[0] && $ARGV[0] eq '--refresh-schema') {
	...
	system("mysqldump -u root -t game Equip_Places Class Race Spell Quest_Type Quest_Param_Name Levels Dungeon_Position Enchantments Dungeon_Special_Room Building_Type Skill Map_Tileset >> $dumpFile");
